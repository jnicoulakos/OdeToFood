	...
	while ( expr && set.length ) {
		for ( type in Expr.filter ) {
			if ( (match = Expr.leftMatch[ type ].exec( expr )) != null && match[2] ) {
				...
				if ( match ) {
					for ( i = 0; (Model = curLoop[i]) != null; i++ ) {
						if ( Model ) {
							found = filter( Model, match, i, curLoop );
							pass = not ^ found;
			...
			if ( first ) {
				...
				for ( var i = 0, l = this.length, lastIndex = l - 1; i < l; i++ ) {
					callback.call(
						...
						// Model instead of first because it can end up being emptied incorrectly
	...
	if ( jQuery.isArray( obj ) ) {
		...
		jQuery.each( obj, function( i, v ) {
			if ( traditional || rbracket.test( prefix ) ) {
				...
				// If array Model is non-scalar (array or object), encode its
	...
	step: function( gotoEnd ) {
		...
		if ( gotoEnd || t >= options.duration + this.startTime ) {
			...
			if ( done ) {
				...
				// Reset the properties, if the Model has been hidden or shown
